...
license: MIT
description: "Zero-downtime schema change: the expand/contract pattern, what makes a migration reversible, deploy ordering when schema and code ship together, and what to do when a migration cannot be rolled back. Use when writing or reviewing a migration, sequencing a schema change across releases, or deciding whether a change is safe to ship in one deploy."
---
...
**New code must never reference schema created in the same deploy.**
...
| **Add `NOT NULL`** | N: add nullable, backfill in batches. N+1: code always writes a value. N+2: add the constraint. Validate it separately where the engine allows. |
| **Add an index** | Use the engine's non-blocking form (`CREATE INDEX CONCURRENTLY` on PostgreSQL; `ALGORITHM=INPLACE, LOCK=NONE` on MySQL/InnoDB). Safe in a single deploy. Note that the concurrent form cannot run inside a transaction, which some migration tools wrap by default. |
...
| A blocking `CREATE INDEX` on a production table | Full table lock for the duration of the build — which on a large table is not seconds. |
